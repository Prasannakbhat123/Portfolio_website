  ...
  return (
    <header className='w-full px-32 py-8 font-medium flex items-center justify-between dark:text-light relative z-10 lg:px-16 md:!px-12 sm:!px-8'>
      ...
      <button className=' flex-col justify-center items-center hidden lg:flex' onClick={handleClick}>
        <span className={`bg-dark dark:bg-light block transition-all duration-3000 ease-out h-0.5 w-6 rounded-sm
         ${isOpen ? 'rotate-45 translate-y-1' : '-translate-y-0.5'}`}></span>
        <span className={`bg-dark dark:bg-light block transition-all duration-3000 ease-out h-0.5 w-6 rounded-sm
        my-0.5 ${isOpen ? 'opacity-0' : 'opacity-100'}`}></span>
        <span className={`bg-dark dark:bg-light block transition-all duration-3000 ease-out h-0.5 w-6 rounded-sm
         ${isOpen ? '-rotate-45 -translate-y-1' : 'translate-y-0.5'}`}></span>
      </button>
...
<nav>
        ...
        <CustomLink href="/sketches" title="Sketches" className='ml-4'/>
      </nav>
      ...
      <nav className='flex items-center justify-center flex-wrap'>
        ...
        <motion.a href="https://www.instagram.com/prasanna.bhatt_/" target="_blank"
         whileHover={{y:-2}}
         whileTap={{scale:0.9}}
        className='w-6 mx-4'>
        <InstaIcon/>
